"""
Czech Hunter Scraper Adapter for Kodi
Bridges between CzechHunter.py and the Kodi addon structure
"""

import xbmc
from .CzechHunter import czechhunter
...
class CzechHunterScraper:
    """Kodi-compatible Czech Hunter scraper"""
    
    def __init__(self):
        self.domains = ['czechhunter', 'debtdandy', 'dirtyscout']
    
    def search(self, title, year=None):
        """Search for scenes"""
        try:
            result = scene_search(title, search_domains=self.domains, postprocess=czechhunter)
            ...
            if isinstance(result, list):
                for item in result:
                    scenes.append({
                        ...
                        'studio': item.get('studio', {}).get('name', 'Czech Hunter') if isinstance(item.get('studio'), dict) else 'Czech Hunter'
                    })
            
            return scenes
            
        except Exception as e:
            xbmc.log("Czech Hunter search error: {}".format(str(e)), xbmc.LOGERROR)
            return {'error': str(e)}
    
    def get_details(self, scene_id):
        """Get scene details"""
        try:
            url = "https://bigstr.com/scene/{}".format(scene_id)
            ...
            if isinstance(studio, dict):
                info['studio'].append(studio.get('name', 'Czech Hunter'))
            else:
                info['studio'].append('Czech Hunter')
            ...
            return {
                'info': info,
                'cast': cast,
                'uniqueids': {'czechhunter': scene_id},
                'available_art': available_art
            }
            
        except Exception as e:
            xbmc.log("Czech Hunter details error: {}".format(str(e)), xbmc.LOGERROR)
            return {'error': str(e)}
